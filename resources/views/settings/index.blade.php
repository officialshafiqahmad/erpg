    ...
    if ($('#cust-darklayout').length > 0) {
        ...
        custthemedark.addEventListener("click", function() {
            if (custthemedark.checked) {
                $('#main-style-link').attr('href', '{{ config('app.url') }}' + '/public/assets/css/style-dark.css');
                document.body.style.background = 'linear-gradient(141.55deg, #22242C 3.46%, #22242C 99.86%)';

                $('.dash-sidebar .main-logo a img').attr('src', '{{ isset($logo_light) && !empty($logo_light) ?  $logo . $logo_light :  $logo . "/logo-light.png" }}');
                ...
                $('.dash-sidebar .main-logo a img').attr('src', '{{isset($logo_light) && !empty($logo_light) ? $logo . $logo_light : $logo . "/logo-dark.png" }}');
                ...
                {{--  Start for all settings tab --}}
                    ...
                    <div id="email-settings" class="card">
                        <div class="card-header">
                            ...
                            <small class="text-muted">{{ __('This SMTP will be used for system-level email sending. Additionally, if a company user does not set their SMTP, then this SMTP will be used for sending emails.')}}</small>
                        </div>
                        <div class="card-body">

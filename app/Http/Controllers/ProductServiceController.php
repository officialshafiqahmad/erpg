...
class ProductServiceController extends Controller
{
    ...
    public function searchProducts(Request $request)
    {
        ...
        if ($request->cat_id == null) //comment if
        {
            $request->cat_id = "";
        }
    ...
    public function searchProductsSku(Request $request)
    {
        ...
        if (Auth::user()->can('manage pos') && $request->ajax() && isset($lastsegment) && !empty($lastsegment)) {
            ...
            if (isset($cart) && count($cart) > 0) {
                $productsSku = ProductService::whereIn('id', array_keys($cart))->get();
            ...
            if ($request->cat_id !== '' && $request->cash_register_id == '0' && $request->search == '') {
                $ids = WarehouseProduct::where('warehouse_id', 1)->get()->pluck('product_id')->toArray();
                if ($request->cat_id == '0') {
                    $productsSku = ProductService::getallproducts()->whereIn('product_services.id', $ids)->get();
                } else {
                    $productsSku = ProductService::getallproducts()->where('category_id', $request->cat_id)->whereIn('product_services.id', $ids)->get();
                }
            } else {
                if ($request->cat_id == '0') {
                    $productsSku = ProductService::getallproducts()->where('product_services.sku', 'LIKE', "%{$request->search}%")->get();
                } else {
                    $productsSku = ProductService::getallproducts()->where('product_services.sku', 'LIKE', "%{$request->search}%")->orWhere('category_id', $request->cat_id)->get();
                }
            }
            if ($productsSku) {
                foreach ($productsSku as $key => $product) {
                    $image_url = (!empty($product->image) && Storage::exists($product->image)) ? $product->image : 'uploads/logo/placeholder.png';
                    if ($request->session_key == 'purchases') {
                        $productprice = $product->purchase_price != 0 ? $product->purchase_price : 0;
                    } else if ($request->session_key == 'pos') {
                        $productprice = $product->sale_price != 0 ? $product->sale_price : 0;
                            ...
                            <div class="tab-pane fade show active toacart w-100" data-url="' . url('add-to-cart/' . $product->id . '/' . $lastsegment) . '">
                                <div class="position-relative card">
                                    ...
                                    <div class="p-0 custom-card-body card-body d-flex ">
                                        <div class="card-body my-2 p-2 text-left card-bottom-content">
                                            <h6 class="mb-2 text-dark product-title-name">' . $product->name . '</h6>
                                            <p class="mb-2 text-dark product-title-name small">' . $product->sku . '</p>
                ...
                $output = '<div class="card card-body col-12 text-center">
                    <h5>' . __("No SKU Available") . '</h5>
                    </div>';
                return Response(__($output));
            }
        }
    }
